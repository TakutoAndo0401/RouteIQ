...
| Workspace | 役割 |
| --- | --- |
| `packages/contracts` | Web と server が共有する API schema / TypeScript 型。Zod schema を単一の契約として使います。 |
| `web` | React / Vite の画面。single-file build で server または Cloudflare Pages から配信します。 |
| `server` | 経路比較、回答文生成、API handler を担当します。Node server と Cloudflare Pages Functions の両方から同じ処理を使います。 |
| `functions` | Cloudflare Pages Functions 用の API entry。`/api/*` を `server` の共通 handler に渡します。 |
...
| `DEFAULT_FUEL_PRICE_YEN_PER_LITER` | 任意。未指定時の燃料単価 |
